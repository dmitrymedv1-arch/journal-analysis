...
async def fetch_with_retry(session, url, params=None, headers=None, method='GET'):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            if SHOW_DEBUG_LOGS and attempt > 0:
            ...
            async with session.request(method, url, params=params, headers=headers, timeout=TIMEOUT) as response:
                ...
                if response.status == 200:
                    ...
                    if SHOW_DEBUG_LOGS:
                        print(f"⚠️ Not found (404) for {url}")
                    return None
                else:
                    if SHOW_DEBUG_LOGS:
            ...
            if SHOW_DEBUG_LOGS:
                print(f"⏰ Timeout error (attempt {attempt+1}/{MAX_RETRIES}) for {url}")
...
<div class="report-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="section">
            ...
            {f'<p style="margin-top: 10px; color: #666; font-size: 13px;">Showing first 50 of {len(all_publications)} publications. Use filters above to find specific publications.</p>' if len(all_publications) > 50 else ''}
        </div>
